...
import 'package:mobile_ecommerce/Application/usecases/delete_an_address_usecase.dart';
...
void main() {
  ...
  group('Delete An Adress Usecase', () {
    test('user can delete an adresse', (() async {
      User user = User();
      ...
      Delete_An_Address_Usecase delete_an_address_usecase =
          Delete_An_Address_Usecase(address_repository);

      address.set_street_name('Rue Jean Jaures');

      await delete_an_address_usecase.remove_user_address(address);

      var user_actual_addresses_ =
          await address_repository.retrieve_user_addresses(user);

      expect(await user_actual_addresses_.isEmpty, true);
