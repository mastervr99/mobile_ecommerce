...
import 'package:mobile_ecommerce/Application/usecases/update_an_address_usecase.dart';
...
void main() {
  ...
  group('Update An Adress Usecase', () {
    test('user can update an adresse', (() async {
      User user = User();
      ...
      Update_An_Address_Usecase update_an_address_usecase =
          Update_An_Address_Usecase(address_repository);

      address.set_street_name('Rue Jean Jaures');

      await update_an_address_usecase.update_user_address(address);

      var new_user_addresses =
          await address_repository.retrieve_user_addresses(user);

      expect(await new_user_addresses[0].get_street_name(), 'Rue Jean Jaures');
